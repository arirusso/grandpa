module Grandpa::SelectMode
  ...
  class Single
    ...
    def handle_mousedown(models, selection)
      to_select = models.find_all {|o| o.selectable? }
    ...
    def mark_selected(to_select, previous_selection)
      ...
      unless double_click
        previous_selection.each { |model| model.deselect_proc.call({}) }
        previous_selection.clear
      end
      to_select.each { |model| model.select_proc.call(:selection => previous_selection) }
  ...
  class Multi
    ...
    def handle_mousedown(models, selection)
      to_select = models.find_all { |o| o.selectable? }
      to_select.each { |o| o.select_proc.call(:selection => selection) }
      selection << to_select
